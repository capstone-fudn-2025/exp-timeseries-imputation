...
class CombinationMode(enum.Enum):
    ...
    def get_combination_mode(mode: str):
        ...
        elif mode == CombinationMode.MEOW.value:
            return CombinationMode.MEOW
        else:
            raise ValueError(
...
class Combined(Callback):
    ...
    def after_forecast(self, y_true, y_fore):
        ...
        if self.n_times_tracking <= self.n_times // 2:
        ...
        if self.n_times_tracking == self.n_times:
            for i, pipe in enumerate(self.pipeline):
                ...
                # Combination mode: MEAN. Treat the pipeline and reverse pipeline as equal
                if self.combination_mode == CombinationMode.MEAN:
                    combined_pipeline = np.mean(
                        (pipe, reverse_pipe), axis=0)

                # Combination mode: DATA_PER. Treat the pipeline and reverse pipeline as the percentage of data
                ...
                # Combination mode: SIMILARITY. Treat the pipeline and reverse pipeline as the similarity of the data
                ...
                # Combination mode: MEOW. Treat the pipeline and reverse pipeline as the gradient function
                elif self.combination_mode == CombinationMode.MEOW:
                    e = 0.1
                    i = np.arange(pipe.shape[0])
                    alpha = - 1/pipe.shape[0] * i * (1-2*e) + (1-e)
                    combined_pipeline = alpha * pipe + \
                        (1 - alpha) * reverse_pipe
