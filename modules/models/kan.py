...
class KAN(BaseModelWrapper):
    '''
    Kolmogorov-Arnold Network for Time Series Forecasting
    '''
    # Define the name of your model. This name will display during training or plotting results.
    name = "KAN"

    def __init__(self, units: list[int] = [24, 1], epochs: int = 20, **kwargs):
    ...
    def fit(self, generator, x, y):
        ...
        self.model = KANModel(
            layers_hidden=[generator.window_size, *self.units])

        progress = tqdm.tqdm(generator, total=len(generator))
        ...
        for epoch in range(self.epochs):
            ...
            # Average loss
            self.history.append(running_loss / len(generator))
    ...
    def predict(self, generator, x):
        ...
        return torch.cat(y_pred).squeeze().detach().numpy()
