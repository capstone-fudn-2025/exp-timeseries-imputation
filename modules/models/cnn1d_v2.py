...
class CNN1D(nn.Module):
    def __init__(self, n_features: int, window_size: int, config: dict):
        ...
        self.conv0_features = config.get('conv0_features', 128)
        self.conv0_kernel_size = config.get('conv0_kernel_size', 5)

        self.conv1_features = config.get('conv1_features', 128)
...
class PhuLienCNN1D(BaseModelWrapper):
    """
    CNN1D model for time series forecasting.
    """
    name = 'cnn1d_v2'
    ...
    def __init__(self, n_features: int = 1, **kwargs):
        ...
        self.learning_rate = kwargs.get('lr', 2e-4)
