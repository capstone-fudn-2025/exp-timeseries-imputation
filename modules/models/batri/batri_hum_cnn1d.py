...
class CNN1D(nn.Module):
    def __init__(self, n_features: int, window_size: int, config: dict):
        ...
        self.conv0_features = config.get('conv1_features', 128)
        self.conv0_kernel_size = config.get('conv1_kernel_size', 3)
        ...
        self.layers = [
            ...
            nn.Conv1d(in_channels=self.n_features, out_channels=self.conv0_features,
                      kernel_size=self.conv0_kernel_size, padding="same"),
            nn.ReLU(),
            ...
            nn.Linear(self.conv2_features * window_size,
...
class BaTriHumCNN1D(BaseModelWrapper):
    """
    CNN1D model for time series forecasting.
    """
    name = 'BaTriHumCNN1D'
